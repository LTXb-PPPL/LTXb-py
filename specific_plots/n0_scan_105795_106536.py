...
def aps_2022_n0_scan():
	ltx_shot = 105795
	fig, ((ax1, ax2), (ax3, ax4)) = plt.subplots(nrows=2, ncols=2, sharex=True, figsize=(12, 5))
	fig.suptitle(f'{ltx_shot} 13 kV NBI')
	fs = 13  # fontsize for legend
	fs2 = 10
	clrs = plt.rcParams['axes.prop_cycle'].by_key()['color']
	ax1r = ax1.twinx()
	# ax1rr = ax1.twinx()
	ax1r.spines["left"].set_position(("axes", -0.2))  # red one
	# ax1rr.spines["left"].set_position(("axes", -0.2))  # green one
	make_patch_spines_invisible(ax1r)
	# make_patch_spines_invisible(ax1rr)
	ax1r.spines["left"].set_visible(True)
	...
	ax3.set_xlabel('n0 (ext) [#/cm^3]', fontsize=fs)
	ax4.yaxis.set_visible(False)
	ax4r = ax4.twinx()
	ax4.set_xlabel('n0 (ext) [#/cm^3]', fontsize=fs)
	ax4r.set_ylabel('Beam Heating [J]', fontsize=fs)
	
	for ax in [ax1, ax2, ax3, ax4, ax1r, ax2r, ax2rr, ax4r]:
		ax.set_xscale('log')
		ax.tick_params(labelsize=fs2)
	shots = [ltx_shot * 10000 + 1406 + i for i in range(5)]  # 6-10
	n0 = np.array([1.e8, 1.e9, 1.e10, 1.e11, 1.e12])
	eb_arr = np.array([13.e3])  # np.array([10.e3, 13.e3, 16.e3])
	# n0, eb = np.meshgrid(n0_arr, eb_arr)  # same method as used in update_trdat
	ls_arr = ['-', '-.', ':']
	# for i in range(len(eb_arr)):
	# 	ax1.plot([np.nan], [np.nan], 'k', ls=ls_arr[i], label=f'{eb_arr[i] / 1000.} kV')
	...
	for i, (eb, ls) in enumerate(zip(eb_arr, ls_arr)):
		...
		vb = vbev / 1000.  # [kV]
		if ltx_shot == 106536:
			pb = np.array([2.87e5] * len(shotselect))
			dt = .46559 - .45834
		elif ltx_shot == 105795:
			pb = np.array([3.24e5] * len(shotselect))
			dt = .4756 - .4684  # toff-ton
		ib = pb / vbev  # [A]
		perv = ib / vbev ** 1.5
		j_tot = pb * dt  # [J] tot put into plasma
		...
		ax1.plot(n0, ib, 'x-', c=clrs[0])
		ax1.set_ylabel('Beam Current [A]', c=clrs[0], fontsize=fs)
		bden_arr = np.array([])  # [num.e12/cm^3]
		bphto_arr = np.array([])  # [kW]
		bphto_arr2 = np.array([])  # [kW]
		bpshi_arr = np.array([])  # [J]
		...
		for shot in shotselect:
			if shot in badshots:
				...
				bphto_arr = np.append(bphto_arr, max(bphto.data) / 1000.)
				bden_arr = np.append(bden_arr, max(bdens.data.flatten()) / 1.e12)
		ax2r.plot(n0, bden_arr, 'o-', c=clrs[0])
		ax2rr.plot(n0, bphto_arr, 'o-', c=clrs[1])
		
		ax3.plot(n0, bpshi_arr / j_tot, 'o-', label='shine-through')
		ax3.plot(n0, bplim_arr / j_tot, 'o-', label='orbit loss')
		ax3.plot(n0, cxi_arr / j_tot, 'o-', label='cx int')
		ax3.plot(n0, cxx_arr / j_tot, 'o-', label='cx ext')
		
		ax4r.plot(n0, bphto_arr2, 'o-', label='total')
		ax4r.plot(n0, bpte_arr, 'o-', label='elec')
		ax4r.plot(n0, bpti_arr, 'o-', label='ions')
	
	ax3.legend(fontsize=fs2)
	ax4r.legend(fontsize=fs2)
	plt.tight_layout()
