; Capecchi 8Jul22
; rmode is now psimode, since nested circular flux surfaces of MST do
; not apply to LTX


; DO I WANT TO PORT THIS TO PYTHON?
; I can send over orbits (.sav) and a reconstruction (.eqdsk) and
; compute vdote from there


PRO phase_scan_ltx,stop=stop,mnum=mnum,nnum=nnum,fepm=fepm,psimode=psimode, $
                   verbose=verbose,parallel=parallel,ps=ps,click=click, $
                   newdata=newdata,contours=contours,thesis=thesis,phi0=phi0, $
                   reclick=reclick
...
if not keyword_set(mnum) then m=1 
if not keyword_set(nnum) then n=2 

IF NOT keyword_set(psimode) THEN psimode = (findgen(nr+1)/nr)*.35
IF NOT keyword_set(fepm) THEN fepm = (findgen(nf+1)/nf-.5)*150.e3 + 100.e3  ; 25-175kHz
;IF NOT keyword_set(phi0) THEN phi0 = (findgen(no+1)/no)*2.*!pi
IF NOT keyword_set(phi0) THEN phi0 = 0.

orbit_dir = '/home/capecchi/resonance/ltx_orbits/'
orbits = file_search(orbit_dir+'*orbit.sav')
;orbits = shift(orbits,12)
...
f_orb = make_array(3,n_elements(orbits)) ;orbit predicted fepm for l=-1,0,1
ioniz0 = make_array(4, n_elements(orbits)) ; x,y,R,z of ionization pt for each orbit
gc0 = make_array(4, n_elements(orbits)) ; x,y,R,z of guiding center pt for each orbit at ionization

FOR o=0,n_elements(orbits)-1 DO BEGIN
...
;          IF zoom_level_1 THEN BEGIN
;            rmode = (findgen(nr+1)/nr-.5)*.05 + resonance.rmode[resonance.rmax]
;            fepm = (findgen(nf+1)/nf-.5)*5.e3 + resonance.fepm[resonance.fmax]
;            phi0 = (findgen(no+1)/no-.5)*!pi/3. + resonance.phi0[resonance.omax]
;              ENDIF
