...
# extract individual wire data
def get_wire_segment_data(data: pd.DataFrame) -> pd.DataFrame:
    xtime = data['Time (s)'].values
    fig, (ax1, ax2) = plt.subplots(ncols=2)
    channel_cols = [c for c in data.columns if c not in ["Time (s)", "Comment"]]
    for col in channel_cols:
        if '16' not in col:
            ax2.plot(xtime, data[col].values, label=col)

    w1 = data['Untitled'].values - data['Untitled 1'].values
    w2 = data['Untitled 1'].values - data['Untitled 2'].values
    w3 = data['Untitled 2'].values - data['Untitled 3'].values
    w4 = data['Untitled 3'].values - data['Untitled 4'].values
    w5 = data['Untitled 4'].values - data['Untitled 5'].values
    w6 = data['Untitled 5'].values - data['Untitled 6'].values
    w7 = data['Untitled 6'].values - data['Untitled 7'].values
    w8 = data['Untitled 7'].values
    w9 = -data['Untitled 8'].values
    w10 = data['Untitled 9'].values - data['Untitled 8'].values
    w11 = data['Untitled 10'].values - data['Untitled 9'].values
    w12 = data['Untitled 11'].values - data['Untitled 10'].values
    w13 = data['Untitled 12'].values - data['Untitled 11'].values
    w14 = data['Untitled 13'].values - data['Untitled 12'].values
    w15 = data['Untitled 14'].values - data['Untitled 13'].values
    w16 = data['Untitled 15'].values - data['Untitled 14'].values

    for iw, w in enumerate([w1, w2, w3, w4, w5, w6, w7, w8, w9, w10, w11, w12, w13, w14, w15, w16]):
        ax1.plot(xtime, w, label=iw + 1)
    for ax in [ax1, ax2]:
        ax.set_xlim((250, 280))
        ax.legend()
    plt.show()
...
def build_power_table(data: pd.DataFrame) -> pd.DataFrame:
    ...
    for icol, col in enumerate(channel_cols):
        icol = icol % len(clrs)
        ...
        P_per_area = TUNGSTEN_DENSITY * WIRE_LENGTH * SPECIFIC_HEAT * (delta_T / delta_t) if delta_t > 0 else float(
            "line 90 issue")
...
wire_df = get_wire_segment_data(data)
