...
def average_multiple_shots(ltx_shots, dateshot=None):
	ton, toff, einj, pinj = [], [], [], []
	...
	for shot in ltx_shots:
		try:
			...
			if time_arr is None:
				time_arr = tbeam
			vbeam = np.interp(time_arr, tbeam, vbeam)
			ibeam = np.interp(time_arr, tibeam, ibeam)
			tbeamon, tbeamoff = tbeam[np.where(vbeam > 1000)][0] + pad, tbeam[np.where(vbeam > 1000)][-1] - pad
			ton.extend([tbeamon])
			toff.extend([tbeamoff])
			einj.append(vbeam)
			pinj.append(ibeam * vbeam)
		except:
	...
	if max(ton) - min(ton) > 1.e-3:
		...
		checkshots = True
	if max(toff) - min(toff) > 1.e-3:
		print('discrepancy in beam turnoff times, check shots')
		checkshots = True
	pb = np.average(np.array(pinj), axis=0)
	eb = np.average(np.array(einj), axis=0)
	ton, toff = np.nanmean(ton), np.nanmean(toff)
	...
	pb *= transmission_efficiency * neutralization_efficiency
	
	ii = np.where((time_arr >= ton + pad) & (time_arr <= toff - pad))
	pavg, eavg = np.nanmean(pb[ii]), np.nanmean(eb[ii])
	print(f'computed avg {pavg:.2f} W : {eavg:.2f} eV : {ton:.5f} beamon : {toff:.5f} beamoff')
	
	return time_arr, pb, eb, shotdate, np.nanmean(ton), np.nanmean(toff)
...
def write_nbi_ufile_multishot(direc=None, trshot=None, tralphanum=None, shots=None):
	...
	tstrt, tend = get_trdat_timing(trdat)
	tdat, pdat, edat, shotdate, tbon, tboff = average_multiple_shots(shots, dateshot=trshot)
	...
	tofile = f'{direc}PVN{trshot}.NBI'  # "time varying neutral beam"
	nbeams = 1  # FOR NOW!! Will LTX ever see a 2nd beam installed? Stay tuned!!
	nt = len(t)
	nchan = 4 * nbeams  # pbeam, ebeam, ffull, fhalf
	
	content = f' {trshot}LTX  2 0 6             ; SHOT\n'
	content += f' {shotdate}               ;SHOT DATE\n'
	content += '   1                           ;Number of associated scalar quantities\n'
	content += f'  {nbeams:.4E}                         ;Scalar, label follows:\n'  # format to match test Ufile
	content += f' NBEAM:    # OF BEAMS          \n'
	...
	content += ';-------END OF DATA-------------\n'
	content += f'Written by python routine on {datetime.datetime.today().strftime("%d %b %Y")}\n'
	content += f'Data aggregated from shots {shots}\n'
	content += f'Author: Bill Capecchi wcapecch@pppl.gov'
...
if __name__ == '__main__':
	...
	# shots = [108886, 108887, 108890, 108880, 108874, 108881, 108895, 108891, 108876, 108875, 108894]
	# write_nbi_ufile_multishot(direc=f'Z:/transp/t108890/', trshot=108890, tralphanum='P01', shots=shots)
	
	shots = [107817, 107818, 107819, 107820, 107785, 107794, 107795, 107806, 107786, 107789, 107791, 107800, 107801,
	         107807, 107808, 107809, 107815, 107816, 107822]
	write_nbi_ufile_multishot(direc=f'Z:/transp/t107801/',trshot=107801,tralphanum='C01',shots=shots)
